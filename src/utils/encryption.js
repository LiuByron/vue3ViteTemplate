...
var doBlockCrypt = function doBlockCrypt(blockData, roundKeys) {
  ...
  for (var i = 0; i < 32; i++) {
    xBlock[i + 4] =
      xBlock[i] ^
      tTransform1(xBlock[i + 1] ^ xBlock[i + 2] ^ xBlock[i + 3] ^ roundKeys[i]);
  }
...
var encrypt_cbc = function encrypt_cbc(plaintext, key, iv) {
  ...
  if (mode === "base64") {
    ...
    return decodeURIComponent(
      escape(String.fromCharCode.apply(String, outArray))
    );
  }
};
/**
...
export var encrypt_ecb = function encrypt_ecb(plaintext, key) {
  var mode =
...
var decrypt_cbc = function decrypt_cbc(ciphertext, key, iv) {
  ...
  return decodeURIComponent(
    escape(
      String.fromCharCode.apply(String, _toConsumableArray(depaddedPlaintext))
    )
  );
};
/**
